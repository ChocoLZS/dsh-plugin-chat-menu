...
> ⚠️ **同一时间只装一种形态**：动态插件与 bundle 插件（`dsh plugin add`）都注册 `@` 文件菜单。动态版 overlay id `at-file-menu`、bundle 版 `chat-menu`——如果两者同时运行，输入 `@` 会出现两个菜单。请二选一。
...
| 浏览器半 | 闭包符号（React/host） | module-loader bundle（require('react') + fetch） |
| 适用 | 未发布 npm / 临时调试 | 正式安装、多机器分发 |

业务逻辑与 UI 两者共用同一份 `src/core/`（见仓库根 README「🧬 单一源码保证」）。
