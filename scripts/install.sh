...
# - 回滚：dsh plugin --profile web remove dsh-plugin-chat-menu。
# =============================================================================
set -euo pipefail
...
if [ "$DRY_RUN" = true ]; then
  ...
  say "[dry-run] 步骤 4：幂等移除 $PATCH_YML 里旧的 chat-menu 手动挂载行（避免双挂载）"
  if [ "$RESTART" = true ]; then say "[dry-run] 步骤 5：pm2 restart dsh-web"; else say "[dry-run] 步骤 5：提示用户手动重启 DSH"; fi
  exit 0
fi

# 步骤 1：预写 workspace 设置（幂等），放行发布 <24h 的新版本
WS_RESULT="$(node -e '
const fs = require("fs");
const p = process.argv[1];
let t = fs.readFileSync(p, "utf8");
const before = t;
if (!/^\s*-\s+dsh-plugin-chat-menu\s*$/m.test(t)) {
...
while (i < lines.length) {
  ...
  if (/^[ \t]*- insert:\s*$/.test(line)) {
    ...
    if (block.some((l) => /id:\s*chat-menu\b/.test(l))) {
...
[ "$MOUNT_RESULT" = "removed" ] \
  && say "已从 $PATCH_YML 移除旧的 chat-menu 手动挂载行（bundle 通道接管挂载）" \
  || say "无旧手动挂载行，跳过"
